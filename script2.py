...
START = "2026-6-10 14:00:00"
END   = "2026-6-10 14:20:00"

OUT_DIR = Path(__file__).parent / "nvr_videos"
FIND_PAGE_SIZE = 50
MIN_DAV_BYTES = 50_000
LARGE_DOWNLOAD_BYTES = 100_000_000
# Intervalo > 1 h: janelas de download (1 = uma requisição loadfile por hora)
CHUNK_HOURS = 1
# Se CHUNK_HOURS = 0, usa janelas de CHUNK_MINUTES alinhadas a :00/:30
CHUNK_MINUTES = 30
# Janelas fixas de download (não uma requisição por arquivo de movimento no NVR)
LOADFILE_SUB_MINUTES = 10
# Intervalo ≤ este valor: um loadfile alinhado à gravação; acima: janelas de 10 min
SHORT_INTERVAL_MAX_MINUTES = 30
# Abaixo disso o loadfile do MHDX costuma retornar HTTP 400
MIN_CLIP_SECONDS = 60
...
def iter_time_chunks(start: str, end: str, hours: int):
    yield from iter_time_chunks_minutes(start, end, hours * 60)
...
def iter_aligned_halfhour(start: str, end: str):
    """Janelas de até 30 min alinhadas a :00/:30 (como os blocos .dav no disco)."""
    cur = parse_dt(start)
    stop = parse_dt(end)
    while cur < stop:
        if cur.minute < 30:
            boundary = cur.replace(minute=30, second=0, microsecond=0)
        else:
            boundary = (cur.replace(minute=0, second=0, microsecond=0) + timedelta(hours=1))
        if boundary <= cur:
            boundary = cur + timedelta(minutes=CHUNK_MINUTES)
...
def sort_dav_paths(paths: list[Path]) -> list[Path]:
    def key(path: Path) -> tuple[int, int, str]:
        m = re.match(r"(?:chunk|block|win)_(\d+)(?:_(\d+))?\.dav$", path.name)
        if m:
...
class NVRClient:
    ...
    def _download_aligned_chunks(
        ...
        if CHUNK_HOURS > 0:
            windows = list(iter_time_chunks(start, end, CHUNK_HOURS))
            label = f"{CHUNK_HOURS} h"
        else:
            windows = list(iter_aligned_halfhour(start, end))
            label = f"{CHUNK_MINUTES} min alinhadas"
        total = len(windows)
        print(f"loadfile em janelas de {label} ({total} parte(s))...")
        downloaded: list[Path] = []
        failed: list[tuple[int, str, str]] = []

        for i, (chunk_start, chunk_end) in enumerate(windows):
            part = segments_dir / f"chunk_{i:04d}.dav"
            if part.exists() and part.stat().st_size == 0:
                part.unlink()
            print(f"  [{i + 1}/{total}] {chunk_start} → {chunk_end}")
            size = self.download_clip_resilient(
                channel, chunk_start, chunk_end, part
            )
            if size:
                downloaded.append(part)
            else:
                failed.append((i, chunk_start, chunk_end))
                print("    falhou — será re-tentado em 10 min")

        for chunk_idx, chunk_start, chunk_end in failed:
            sub_windows = list(iter_time_chunks_minutes(chunk_start, chunk_end, 10))
            print(
                f"  trecho {chunk_idx:04d} em 10 min ({len(sub_windows)} parte(s)): "
                f"{chunk_start} → {chunk_end}"
            )
            for sub_idx, (sub_start, sub_end) in enumerate(sub_windows):
                part = segments_dir / f"chunk_{chunk_idx:04d}_{sub_idx}.dav"
                if part.exists() and part.stat().st_size == 0:
                    part.unlink()
                print(f"    [{sub_idx + 1}/{len(sub_windows)}] {sub_start} → {sub_end}")
                size = self.download_clip_resilient(
                    channel, sub_start, sub_end, part
                )
                if size:
                    downloaded.append(part)
                else:
        ...
        return sort_dav_paths(downloaded)
    ...
    def _download_one_block(
        ...
        if expected_interval_seconds(seg_start, seg_end) < MIN_CLIP_SECONDS:
            dbg(f"bloco {seg_start} → {seg_end} muito curto, ignorado")
            return []
        ...
        print("    loadfile falhou — tentando bloco original do NVR (RPC)...")
        try:
    ...
    def _download_time_windows(
        self,
        channel: int,
        default_rec: Recording,
        blocks: list[tuple[Recording, str, str]],
        start: str,
        end: str,
        out_dir: Path,
        ...
        for old_mp4 in segments_dir.glob("*.mp4"):
            old_mp4.unlink(missing_ok=True)

        windows = list(iter_time_chunks_minutes(start, end, LOADFILE_SUB_MINUTES))
        total = len(windows)
        print(
            f"download em {total} janela(s) de {LOADFILE_SUB_MINUTES} min "
            f"(gravação por movimento: lacunas são puladas)..."
        )

        segments: list[DavSegment] = []
        skipped = 0
        for i, (w_start, w_end) in enumerate(windows):
            clipped = clip_window_to_recordings(blocks, w_start, w_end)
            if clipped is None:
                print(f"  [{i + 1}/{total}] {w_start} → {w_end} — sem gravação, pulando")
                skipped += 1
                continue

            clip_start, clip_end = clipped
            if expected_interval_seconds(clip_start, clip_end) < MIN_CLIP_SECONDS:
                skipped += 1
                continue

            part = segments_dir / f"win_{i:04d}.dav"
            if clip_start != w_start.strip() or clip_end != w_end.strip():
                print(
                    f"  [{i + 1}/{total}] {w_start} → {w_end} "
                    f"(gravação: {clip_start} → {clip_end})"
                )
            else:
                print(f"  [{i + 1}/{total}] {clip_start} → {clip_end}")
            segment = self._try_loadfile_window(
                channel, default_rec, clip_start, clip_end, part
            )
            if segment:
        ...
        if skipped:
            print(f"  ({skipped} janela(s) sem gravação ou muito curta(s))")
        return segments

    def _download_short_interval(
        self,
        channel: int,
        default_rec: Recording,
        blocks: list[tuple[Recording, str, str]],
        ...
        if clipped is None:
            return []
        clip_start, clip_end = clipped
        if clip_start != start.strip() or clip_end != end.strip():
            print(f"intervalo ajustado à gravação: {clip_start} → {clip_end}")
        print("baixando intervalo (1 requisição loadfile)...")
        segment = self._try_loadfile_window(channel, default_rec, clip_start, clip_end, dest)
        if segment:
            return [segment]
        return []

    def _download_block_parts(
        self,
        channel: int,
        blocks: list[tuple[Recording, str, str]],
        segments_dir: Path,
        *,
        ...
        segments_dir.mkdir(exist_ok=True)
        total = len(blocks)
        downloaded: list[DavSegment] = []
        for i, (rec, seg_start, seg_end) in enumerate(blocks):
            part = segments_dir / f"{prefix}_{i:04d}.dav"
            print(f"  [{i + 1}/{total}] {seg_start} → {seg_end}")
            segments = self._download_one_block(channel, rec, seg_start, seg_end, part)
            if not segments:
                print("    falhou")
                continue
            for segment in segments:
                downloaded.append(segment)
                if segment.already_clipped:
                    warn_if_short_media(
                        segment.path,
                        segment.seg_start,
                        segment.seg_end,
                        label=segment.path.name,
                    )
        return downloaded
    ...
    def download_interval(
        ...
        """Baixa START→END; retorna segmentos .dav com metadados para conversão."""
        out_dir.mkdir(parents=True, exist_ok=True)
        default_rec = self.filter_channel(recs, channel)[0]

        single = out_dir / "recorte.dav"
        ...
        duration_sec = expected_interval_seconds(start, end)

        if duration_sec <= SHORT_INTERVAL_MAX_MINUTES * 60:
            segments = self._download_short_interval(
                channel, default_rec, blocks, start, end, single
            )
            if not segments:
                print("download único falhou — tentando janelas de 10 min...")
                segments = self._download_time_windows(
                    channel, default_rec, blocks, start, end, out_dir
                )
            if segments:
                for segment in segments:
                    if segment.already_clipped:
                ...
                return segments
            raise RuntimeError(
                "download falhou para o intervalo pedido. Verifique START/END e CHANNEL."
            )

        if duration_sec > LOADFILE_SUB_MINUTES * 60:
            segments = self._download_time_windows(
                channel, default_rec, blocks, start, end, out_dir
            )
            if not segments:
                raise RuntimeError(
                    "Nenhuma janela baixada. Rode de novo — arquivos em segments/ "
                    "são reaproveitados."
                )
        ...
        segments_dir = out_dir / "segments"
        segments_dir.mkdir(exist_ok=True)
        for stale in segments_dir.glob("*.dav"):
            if stale.stat().st_size == 0:
                stale.unlink()

        part_paths = self._download_aligned_chunks(channel, start, end, segments_dir)
        if not part_paths:
            raise RuntimeError(
                "Nenhum segmento baixado. Rode de novo para retomar — arquivos já "
                "baixados em segments/ são reaproveitados."
            )

        if CHUNK_HOURS > 0:
            expected_parts = max(1, int(duration_h / CHUNK_HOURS))
        else:
            expected_parts = max(1, int(duration_h * (60 / CHUNK_MINUTES)))
        if len(part_paths) < expected_parts:
            print(
                f"\nAVISO: {len(part_paths)}/{expected_parts} parte(s) baixada(s). "
                "Rode o script de novo para retomar os trechos faltantes."
            )
        else:
            print(f"\n{len(part_paths)} parte(s) baixada(s) via loadfile")

        return [read_segment_meta(path, default_rec) for path in sort_dav_paths(part_paths)]
            ...
            if dur:
                ...
                if dur < expected_total - 120:
                    print(
                        ...
                        "confira se todas as janelas em segments/ foram baixadas."
                    )
        return 0
